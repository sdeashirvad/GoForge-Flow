    ...
    <div className="space-y-0">
      {retries.map((r, i) => (
        <div key={r.id} className="relative flex gap-4 pb-6 last:pb-0">
          ...
          <div className="relative z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 border-red-500/50 bg-red-950/50">
            <span className="text-xs font-medium text-red-400">{r.attempt_num}</span>
          </div>
          <div className="flex-1 min-w-0 pt-0.5">
            ...
            <p className="text-sm text-red-400 font-mono mt-1 break-all">{r.error_msg}</p>
            {r.backoff_secs > 0 && (
